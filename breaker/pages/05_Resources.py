...
RESOURCE_CATEGORIES = ["Competition", "Engineering", "Design", "Electrical", "Administrative", "Outreach", "Safety",
                       "Training", "Other"]
...
def delete_resource(resource_id):
    ...
    resource_to_delete = next((r for r in resources if r['id'] == resource_id), None)
...
with tab1:
    ...
    if not ("All" in filter_category or len(filter_category) == 0):
        filtered_resources = [resource for resource in filtered_resources if
                              resource.get("category", "Other") in filter_category]

    # File type filter
    if not ("All" in filter_type or len(filter_type) == 0):
        filtered_resources = [resource for resource in filtered_resources if
                              resource.get("file_type", "Other") in filter_type]
    ...
    if filtered_resources:
        ...
        if 'selected_resource' in st.session_state:
            ...
            if selected:
                ...
                with col1:
                    # Actually download the file if it exists
                    if selected.get('file_path') and os.path.exists(selected.get('file_path')):
                        with open(selected.get('file_path'), 'rb') as file:
                            st.download_button(
                                label="Download File",
                                data=file,
                                file_name=os.path.basename(selected.get('file_path')),
                                mime="application/octet-stream"
                            )
                    else:
                        st.button("Download File (Not Available)")
...
if st.session_state.show_resource_form:
    ...
    if editing:
        resource_to_edit = next(
            (resource for resource in resources if resource["id"] == st.session_state.editing_resource), None)
        if resource_to_edit:
    ...
    with st.form("resource_form"):
        # Pre-fill values if editing and resource exists
        if editing and resource_to_edit:
            title_value = resource_to_edit["title"]
            description_value = resource_to_edit.get("description", "")
            category_index = RESOURCE_CATEGORIES.index(resource_to_edit.get("category")) if resource_to_edit.get(
                "category") in RESOURCE_CATEGORIES else -1
            file_type_index = FILE_TYPES.index(resource_to_edit.get("file_type")) if resource_to_edit.get(
                "file_type") in FILE_TYPES else -1
        ...
        with col1:
            resource_category = st.selectbox("Category", RESOURCE_CATEGORIES,
                                             index=category_index if category_index >= 0 else 0)
            resource_tags = st.text_input("Tags (comma separated)", value=tags_value,
                                          help="Example: rules, engine, safety")

        with col2:
            resource_file_type = st.selectbox("File Type", FILE_TYPES,
                                              index=file_type_index if file_type_index >= 0 else 0)
            resource_file_size = st.text_input("File Size", value=file_size_value, help="Example: 2.5 MB")

        # Setup the uploads directory if it doesn't exist
        UPLOADS_DIR = "data/resource_uploads"
        if not os.path.exists(UPLOADS_DIR):
            os.makedirs(UPLOADS_DIR)

        # File upload - now with actual file saving
        uploaded_file = None
        upload_path = None

        if not editing:
            uploaded_file = st.file_uploader("Upload File",
                                             type=["pdf", "doc", "docx", "xls", "xlsx", "csv", "ppt", "pptx", "txt",
                                                   "zip"])
                ...
                if editing:
                    ...
                    for resource in resources:
                        if resource["id"] == st.session_state.editing_resource:
                            # Keep the existing file_path
                            original_file_path = resource.get("file_path")
                            ...
                            # Make sure we preserve the file path
                            if original_file_path:
                                resource["file_path"] = original_file_path
                            break

                    success_message = "Resource updated successfully!"
                else:
                    # Validate that a file is uploaded for new resources
                    if not uploaded_file and not editing:
                        ...
                        if uploaded_file:
                            # Get file extension and determine file type if not specified
                            ...
                            unique_filename = f"{resource_id}_{timestamp}{file_ext}"
