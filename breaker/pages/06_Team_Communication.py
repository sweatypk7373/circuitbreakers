...
if st.session_state.view_thread:
        ...
        with st.container():
            ...
            if st.session_state.edit_message == thread_parent['id']:
                ...
                with st.form(key=f"edit_message_form_{thread_parent['id']}"):
                    ...
                    edit_category = st.selectbox("Category", MESSAGE_CATEGORIES, index=MESSAGE_CATEGORIES.index(
                        thread_parent.get('category', 'Discussion')) if thread_parent.get(
                        'category') in MESSAGE_CATEGORIES else 0)
                    edit_priority = st.selectbox("Priority", MESSAGE_PRIORITIES, index=MESSAGE_PRIORITIES.index(
                        thread_parent.get('priority', 'Normal')) if thread_parent.get(
                        'priority') in MESSAGE_PRIORITIES else 1)
    ...
    channel_messages = [msg for msg in messages if
                        msg.get('channel') == st.session_state.selected_channel and not msg.get('parent_id')]
    ...
    if filter_time == "Today":
        filtered_messages = [msg for msg in filtered_messages if
                             datetime.fromisoformat(msg.get('timestamp')).date() == now.date()]
    elif filter_time == "Past Week":
        filtered_messages = [msg for msg in filtered_messages if
                             datetime.fromisoformat(msg.get('timestamp')) > (now - timedelta(days=7))]
    elif filter_time == "Past Month":
        filtered_messages = [msg for msg in filtered_messages if
                             datetime.fromisoformat(msg.get('timestamp')) > (now - timedelta(days=30))]
            ...
            with st.container():
                ...
                if message.get('author') == st.session_state.user or st.session_state.role in ['admin', 'lead']:
                    with col3:
                        ...
                        action = st.selectbox("Actions", ["Select Action", "Edit", "Delete"],
                                              key=f"action_{message['id']}")
