...
if st.session_state.show_member_form:
    ...
    with st.form("member_form"):
        ...
        with col2:
            role = st.selectbox("Role", TEAM_ROLES, index=TEAM_ROLES.index(user_to_edit.get('role', 'member')) if user_to_edit.get('role') in TEAM_ROLES else 2)
            department = st.selectbox("Department", [""] + TEAM_DEPARTMENTS, index=(TEAM_DEPARTMENTS.index(user_to_edit.get('department')) + 1) if user_to_edit.get('department') in TEAM_DEPARTMENTS else 0)
